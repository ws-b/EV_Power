...
def objective(trial, X_train, y_train, X_val, y_val):
    # 하이퍼파라미터 제안: reg_lambda, reg_alpha, learning_rate만 최적화
    reg_lambda = trial.suggest_float('reg_lambda', 1e-6, 1e+2, log=True)  # 범위 조정 가능
    reg_alpha = trial.suggest_float('reg_alpha', 1e-6, 1e+2, log=True)  # 범위 조정 가능
    ...
    # 파라미터 설정: reg_lambda, reg_alpha, learning_rate만 설정하고 나머지는 기본값 사용
    params = {
...
def bayesian_optimization(trial_func, X_train, y_train, X_val, y_val, n_trials=50):
    ...
    이번 튜닝에서는 reg_lambda, reg_alpha, learning_rate만 최적화합니다.
    """
...
def train_model(X_train, y_train, X_val, y_val, best_params):
    """
    최적의 하이퍼파라미터로 LightGBM 모델을 훈련시킵니다.
    """
    # LightGBM 데이터셋 생성
    train_set = lgb.Dataset(X_train, label=y_train)
    val_set = lgb.Dataset(X_val, label=y_val, reference=train_set)
...
def cross_validate(vehicle_files, selected_car, params=None, plot=False):
    ...
    for fold_num, (train_index, test_index) in enumerate(kf.split(files), 1):
        ...
        # 모델 훈련
        gbm = train_model(X_train, y_train, X_val, y_val, best_params)

        # 예측 수행
        train_data['y_pred'] = gbm.predict(X_train, num_iteration=gbm.best_iteration)
        test_data['y_pred'] = gbm.predict(X_test, num_iteration=gbm.best_iteration)
        ...
        print(
            f"Best Params: reg_lambda={best_params['reg_lambda']:.5f}, reg_alpha={best_params['reg_alpha']:.5f}, "
    ...
    if len(results) == kf.get_n_splits():
        ...
        print(
            f"Best Params for Selected Fold: reg_lambda={best_params_overall['reg_lambda']:.5f}, "
            f"reg_alpha={best_params_overall['reg_alpha']:.5f}, "
            f"learning_rate={best_params_overall['learning_rate']:.5f}"
        )
    else:
        best_model = None
