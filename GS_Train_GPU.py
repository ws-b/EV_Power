...
def plot_3d(X, y_true, y_pred):
    ...
    ax.scatter(X[:, 0], X[:, 1], y_true, c='blue', label='Actual Residual', alpha=0.6, edgecolors='w', s=50)
    ax.scatter(X[:, 0], X[:, 1], y_pred, c='red', label='Predicted Residual', alpha=0.6, edgecolors='w', s=50)
    ax.set_xlabel('Speed')
